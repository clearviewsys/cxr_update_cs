...
$downloadPath:=getKeyValue("au.download.path"; System folder:C487(Desktop:K41:16)+"CXR_NewRelease"+Folder separator:K24:12)
...
FOLDER LIST:C473($downloadPath; $arrTmp)
For ($i; 1; Size of array:C274($arrTmp))
	DELETE FOLDER:C693($downloadPath+$arrTmp{$i})
End for 
...
For ($i; 1; Size of array:C274($arrFilesPtr->))
	...
	If (Not:C34($abort))
		...
		$fileName:=$downloadPath+getFileNameFromURL($arrFilesPtr->{$i})
		...
		UTIL_Log("AutomaticUpdates"; "Document downloaded: "+$fileName)
