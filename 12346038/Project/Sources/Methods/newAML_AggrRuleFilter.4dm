...
// Note: The filter object holds only the filters and parameters that getTransactionStats requires
// Note: This is not a comprehensive object that has all the rule data members inside it, it is only
// for example, EFT, Cash, Receive or Paid, Currency, Account, etc do make a difference in getTransactionStats
