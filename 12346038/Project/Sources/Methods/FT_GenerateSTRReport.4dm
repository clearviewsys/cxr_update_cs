...
While ($i<=$max)
	//QUERY([Invoices];[Invoices]InvoiceID=[AML_Reports]invoiceID)
	...
	///QUERY([Registers]; & ;[Registers]isTransfer=False)
	//QUERY([Registers]; & ;[Registers]isExported=False)
	//QUERY([Registers];[Registers]InvoiceNumber=[Invoices]InvoiceID)
