...
If (isValidationConfirmed)
	...
	$status:=ftpUpload($supportFilePath; [ServerPrefs:27]ftpUploadPath:41; [ServerPrefs:27]ftpHostName:37; [ServerPrefs:27]ftpUserName:38; [ServerPrefs:27]ftpPassword:39; [ServerPrefs:27]ftpUseSecure:114)
	
	If ($status.success)
		myAlert("Upload successfull!")
	Else 
		myAlert("Upload failed!"+CRLF+" Error: "+String:C10($status.status)+CRLF+<>CheckErrors)
	End if 
