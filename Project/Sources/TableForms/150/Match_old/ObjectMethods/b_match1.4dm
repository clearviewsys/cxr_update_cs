...
Case of 
	: (($customerID#"") & ($thisInvoiceID#"") & ($customerObj#Null:C1517) & ($invoiceObj#Null:C1517))  // there is a customer & Invoice
		$match:=agg_isRuleMatchingThisInvoice($invoiceObj; $customerObj; $ruleObj; True:C214)
	...
	: (($customerID#"") & ($thisInvoiceID="") & ($customerObj#Null:C1517))  // there is a customer but no invoice
		...
		$statsObj:=agg_getStatsObject($filterObj; newDateRange(Current date:C33); True:C214; (cb_connected=1))
		
		Form:C1466.stats:=$statsObj
		$match:=agg_isRuleMatchingThisCustomer($customerObj; $statsObj; $ruleObj)
