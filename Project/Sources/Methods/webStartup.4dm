...
If (webIsLicensed)  //check for licensing here
	If (Application type:C494=4D Local mode:K5:1) | (Application type:C494=4D Server:K5:6)  //dont start on remote for now
		//need to look at remotes actiing as web servers later 
...
If (webIsLicensed) & (WEB Is server running:C1313)
	...
	WAPI_configDzVersion(getKeyValue("web.configuration.dropzone.version"; "5"))  //"5")
	WAPI_acceptedFiles(getKeyValue("web.configuration.dropzone.acceptedfiles"; "image/*"))
	...
	WAPI_maxRequest(Num:C11(getKeyValue("web.configuration.requestsize.max"; "10485760")))  //1024*1024*10)
	...
	$tResult:=$tResult+"maxRequest: "+String:C10((WAPI_maxRequest/1024)/1024)+" || "
