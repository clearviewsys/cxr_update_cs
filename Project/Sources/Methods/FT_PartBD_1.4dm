...
Case of 
	...
	: ($orderingPart="Customers")
		...
		If ($partId="B1")
			// TODO: Fix the Account Number instead of the customer ID
			If (getKeyValue("FT.UseCustomerID"; "YES")="YES")
		...
		Else 
			$account:=FT_StringFormat([Customers:3]CustomerID:1; 30)
			// Get account from Links.
			//READ ONLY([Links])
			
			//QUERY([Links];[Links]LinkID=[eWires]LinkID)
			//QUERY([Links];[Links]CustomerID=[Customers]CustomerID;*)
			//QUERY([Links]; & ;[Links]FirstName=[Customers]FirstName;*)
			//QUERY([Links]; & ;[Links]LastName=[Customers]LastName)
			
			
			
			//If (Records in selection([Links])>0)
	...
	: ($orderingPart="Links")
		...
		Else 
			...
			If (([Customers:3]HomeTel:6#"") & ($isSelf))
				$indHomePhoneNumber:=FT_StringFormat(FT_Clean([Customers:3]HomeTel:6); 20)
			Else 
				$indHomePhoneNumber:=FT_StringFormat(" "; 20)
			End if 
		...
		Else 
			...
			Else 
				...
				If (([Customers:3]DOB:5#!00-00-00!) & ($isSelf))
					$indDateOfBirth:=FT_GetStringDate([Customers:3]DOB:5)
				Else 
					$indDateOfBirth:=FT_StringFormat(" "; 8)
				End if 
		...
		//$account:=FT_StringFormat ([Links]BankAccountNo;30)
		//  //If (getKeyValue ("FT.UseCustomerID";"YES")="YES")
		//  //$account:=FT_StringFormat ([Links]LinkID;30)
		//  //Else 
		//  //$account:=FT_StringFormat (" ";30)
		//  //  //$account:=FT_StringFormat ([Links]BankAccountNo;30)
		//  //End if 
		...
		If ($partId="B1")
			$account:=FT_StringFormat([Links:17]BankAccountNo:31; 30)
		Else 
			If (getKeyValue("FT.UseCustomerID"; "YES")="YES")
