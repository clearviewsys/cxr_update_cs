...
C_BLOB:C604($content)
C_TEXT:C284($tot; $footer; $reportSeqStr)
...
$i:=1
While ($i<=$maxTx)
	$branchRef:=[eWires:13]BranchID:50
	...
	While (($branchAct=$branchRef) & ($i<=$maxTx))
		...
		//$isRelatedIran:=isInvoiceRelatedIran ([eWires]InvoiceNumber;initialDate)
		$isRelatedIran:=isInvoiceRelatedIran([Invoices:5]InvoiceID:1; initialDate)
		...
		FT_Generatepart_BFD_1($fileName; [eWires:13]CustomerID:15; "B1"; "Customers"; True:C214)
		...
		// -----------------------------------------------------------------------------------------------------------
		FT_GeneratePartE1_EFTO($fileName)
		...
		FT_GeneratePartF1EFTO($fileName)
