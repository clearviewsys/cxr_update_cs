...
If ($max>0)
	...
	While (($j<=$max) & (Not:C34(End selection:C36([Invoices:5]))))
		...
		While (($branchAct=$branchRef) & ($j<=$max))
			...
			If (($registersDebit.length>0) & (isCustomerReportable([Customers:3]CustomerID:1)))  // Has the invoice Cash Tansactions? No walk-in Customer?
				...
				$p:=Find in array:C230($arrBranches; [Invoices:5]BranchID:53)
				If ($p<0)
					APPEND TO ARRAY:C911($arrBranches; [Invoices:5]BranchID:53)  //Just add only subheaders with valid invoices
	...
	$tot:=FT_NumberFormat(Size of array:C274($arrBranches); 0; 5; "0"; "RJ")
