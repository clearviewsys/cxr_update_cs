//%attributes = {}
// FT_Get_LCTR_InvoicesByQuery ($dateref;->$invoicesPtr;->$invoices24hIndPtr)
...
//QUERY([Invoices]; & ;[Invoices]isAMLReported=True)

//QUERY SELECTION BY FORMULA([Invoices];isDateHourInDateTimeRange ([Invoices]invoiceDate;[Invoices]CreationTime;$dayBefore;vTimeUntil;$refDate;vTimeUntil)=true)
If (False:C215)
	CREATE EMPTY SET:C140([Invoices:5]; "$invoices")
	While (Not:C34(End selection:C36([Invoices:5])))
		If (isDateHourInDateTimeRange([Invoices:5]invoiceDate:44; [Invoices:5]CreationTime:14; $dayBefore; vTimeUntil+?00:00:01?; $refDate; vTimeUntil))
			ADD TO SET:C119([Invoices:5]; "$invoices")
		End if 
		
		NEXT RECORD:C51([Invoices:5])
	End while 
	USE SET:C118("$invoices")
End if 
