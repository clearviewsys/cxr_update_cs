...
C_REAL:C285($rBase; $rDirectRate; $rFromAmount; $rFromFee; $rMargin; $rToAmount; $rInverseRate)
C_REAL:C285($rAmountDue)
	...
	If ($doGetRates)  // tieredRates
		...
		If ($rDirectRate>0)
			...
			If ($quote["dash-quote-pay"]="SQ@")
				...
				If ($sqFee>0)
					$rAmountDue:=$rAmountDue+Round:C94(($rAmountDue*$sqFee); 2)
				End if 
			End if 
