...
For ($i; 1; $n)
	...
	$char:=$str[[$i]]  // small optimization, saves pushing parameters to stack, call to function, pulling result from stack @milan
	
	If (($char="\\") & ($i<$n))
		
		// If (Substring($str; $i+1; 1)="t")  // the next char is a tab
		If ($str[[$i+1]]="t")  // the next char is a tab
			
			postTabKey
			$i:=$i+2  // advance two chars assuming there will be more
			
			If ($i<$n)
				//$char:=Substring($str; $i; 1)  // read the next char after tab
				$char:=$str[[$i]]
			End if 
