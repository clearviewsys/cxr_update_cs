//%attributes = {}
// ------------------------------------------------------------------------------
// FJ_SetGeneralTxDetail_V2 ($fileName;$totTxAmount;$txDate;$debitLC)
// Generate the FIJI FIU Tx Detail
// ------------------------------------------------------------------------------

C_TEXT:C284($1; $fileName; $rptInfo; $purpose; $address; $tmp)
...
If ([Branches:70]LocationNumberFT:19#"")
	$rptInfo:=$rptInfo+":M03:"+[Branches:70]LocationNumberFT:19+CRLF  // Optional
End if 
...
If ([Branches:70]Address:3#"")
	...
	$rptInfo:=$rptInfo+FJ_AddTagToReport(":M04:"; FJ_MaxString($address; 35; 4; False:C215); True:C214)  // Optional
End if 
...
$rptInfo:=$rptInfo+":M08:"+FJ_GetValue(Substring:C12($purpose; 1; 40); "Transaction Description")+CRLF  // Mandatory
$rptInfo:=$rptInfo+":M09:"+FJ_FormatDate($txDate; True:C214)+CRLF  // Mandatory
$rptInfo:=$rptInfo+":M10:"+String:C10($debitLC; "#########0.00")+CRLF  // Optional

If ([Branches:70]BranchPhone:5#"")
	$rptInfo:=$rptInfo+":M15:"+FJ_GetValue([Branches:70]BranchPhone:5)+CRLF  // Optional
End if 

If ([Branches:70]Address:3#"")
	//$rptInfo:=$rptInfo+":M16:"+FJ_GetValue ([Branches]Address)+CRLF 
	$address:=FJ_ContactAddress([Branches:70]Address:3; [Branches:70]City:4; [Branches:70]Province:10; [Branches:70]PostalCode:11; [Branches:70]CountryCode:12)
	$rptInfo:=$rptInfo+FJ_AddTagToReport(":M16:"; FJ_MaxString($address; 35; 4; False:C215); True:C214)  // Optional
End if 
If ([Branches:70]CountryCode:12#"")
	//$rptInfo:=$rptInfo+":M17:"+FJ_GetValue ([Branches]CountryCode)+CRLF 
End if 

$rptInfo:=$rptInfo+":M18:NC"+CRLF  // Optional

// M19:Cash Value. The values for each of the cash types.
$rptInfo:=$rptInfo+":M19:"+String:C10($debitLC; "#########0.00")+CRLF  // Optional
//[Invoices]eWireID
...
If ([Registers:10]isReceived:13)
	$rptInfo:=$rptInfo+":M20:I"+CRLF  // Optional
Else 
	$rptInfo:=$rptInfo+":M20:O"+CRLF  // Optional
End if 
...
// P03: Name: Customer or client conducting the transaction.
$rptInfo:=$rptInfo+":P03:"+FJ_GetValue([Customers:3]FullName:40; "Customer Full Name")+CRLF  // Mandatory
...
$rptInfo:=$rptInfo+FJ_AddTagToReport(":P04:"; FJ_MaxString($address; 35; 4; False:C215); True:C214)  // Mandatory
...
If ([Customers:3]CountryCode:113#"")
	...
	$rptInfo:=$rptInfo+":P08:"+FJ_GetValue([Countries:62]CountryName:2; "Business Country Name")+CRLF  // Mandatory
End if 
...
If ([Customers:3]isCompany:41)
	...
	If ([Customers:3]BusinessType:62#"")
		$rptInfo:=$rptInfo+":P11:"+FJ_GetValue([Customers:3]BusinessType:62; "Business occupation Code")+CRLF  // Mandatory
	End if 
...
Else   // is an Individual
	...
	$rptInfo:=$rptInfo+":P12:"+FJ_FormatDate([Customers:3]DOB:5; True:C214)+CRLF  // Mandatory
	
	// P14: Account Number, P15: Account type, P16: Account type
	//getBuild
	
	FJ_SetAccountDetails(->$rptInfo)
	...
	// Value Meaning
	// A Account
	// B MOJ-Birth Registration Number
	// D LTA-Driver’s license
	// E FRCS-Tax Identification Number
	// F MOJ-National ID Number
	// J FNPF – FNPF ID Card
	// O Other ID
	// P Immigration Department – Passport ID
	// V FEO - V oter Identification Card
...
End if   // else is an individual

// P26: Postal Address
If ([Customers:3]Address:7#"")
	$address:=FJ_ContactAddress([Customers:3]Address:7; [Customers:3]City:8; [Customers:3]Province:9; [Customers:3]PostalCode:10; [Customers:3]CountryCode:113)
	$rptInfo:=$rptInfo+FJ_AddTagToReport(":P26:"; FJ_MaxString($address; 35; 4; False:C215); True:C214)
End if 
...
// P28: Residential Address
If ([Customers:3]Address:7#"")
	$address:=FJ_ContactAddress([Customers:3]Address:7; [Customers:3]City:8; [Customers:3]Province:9; [Customers:3]PostalCode:10; [Customers:3]CountryCode:113)
	$rptInfo:=$rptInfo+FJ_AddTagToReport(":P28:"; FJ_MaxString($address; 35; 4; False:C215); True:C214)
End if 

// P29: Residential Country
If ([Customers:3]CountryOfResidenceCode:114#"")
	$tmp:=getCountryNameByCode([Customers:3]CountryOfResidenceCode:114)
	$rptInfo:=$rptInfo+FJ_AddTagToReport(":P29:"; FJ_MaxString($tmp; 15); True:C214)
End if 

// P30: Residential Phone
If ([Customers:3]HomeTel:6#"")
	$rptInfo:=$rptInfo+":P30:"+FJ_GetValue([Customers:3]HomeTel:6; "Residential Phone  Number Code")+CRLF
End if 

// P32 Temporary Fiji address (optional) 4*35x
// P33 Temporary Fiji phone (optional) 15x
// P34 Account Holder Flag (optional) 1a values Y-Yes N-No
// P35 Account Branch (optional) 4*35x
// P36 Account FI (optional) 4*35x


// TODO: Ask for Q1
// Q01 Transaction type
// Code to indicate type of transaction type involved.
// Value Meaning:
// A Deposit/Withdrawal
// E Foreign Currency & Exchange Transaction
// F Funds Transfer, Remittance, & Card Transaction
// G Trading in Bonds, Securities, Shares, etc.
// H Unit Trust, Trustee & Trust Fund Transaction
// I Loan/Leasing/Credit Funds : Advance/Repayment
// J Real Estate & Investment Transaction
// K Insurance & FNPF Related Transaction
// O Other
...
// N Q04 Financial institution name 30x (optional)
// N Q05 Branch name 30x (optional)
// N Q06 Country name 20x (optional)

// N V01 Free format text n*72x (optional) // Unstructured report text.
// N V02 Additional bank text n*72x (optional)  // Additional information deemed by the financial institution to be relevant to the report.
// N V03 FIJIFIU only text n*72x (optional) // Additional information deemed by FIJIFIU to be relevant to a CTR report.
