...
If (WAPI_isSessionAuthorized($1))  //;webURLContext))
	...
	Case of 
		: (webContext="customer@")  // load so we can use if needed
			READ ONLY:C145(*)
			If ([WebUsers:14]webUsername:1=webUsername)
			Else 
				QUERY:C277([WebUsers:14]; [WebUsers:14]webUsername:1=webUsername)
			End if 
			
			Case of 
				: ([WebUsers:14]recordID:9="")
					REDUCE SELECTION:C351([Customers:3]; 0)
				: ([Customers:3]CustomerID:1=[WebUsers:14]recordID:9)
				Else 
