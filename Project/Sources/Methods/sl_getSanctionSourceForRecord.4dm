		...
		If ($entity.UseServer="")
			If ($entity.Details=Null:C1517)
				...
				If ($entity.ResponseJSON=Null:C1517)
					$result:=sl_useCXBlacklistText
				Else 
					$result:=sl_useCXBlackListJSON
				End if 
			Else 
...
If ($display)
	If (($result=sl_useCXBlacklistText) | ($result=sl_useCXBlackListJSON))
		$result:=sl_useCXBlacklist
	End if 
Else 
	If ($result=sl_useCXBlacklist)
		$result:=sl_useCXBlackListJSON
	End if 
End if 