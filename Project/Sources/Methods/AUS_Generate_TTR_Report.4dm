...
C_TEXT:C284($rootRef; $root; $element; $transcation; $phones; $tmp; $fco)
...
For ($i; 1; $maxTx)
	...
	APPEND TO ARRAY:C911($arrAttrValues; "rbr-"+String:C10($i))
	...
	$arrAttrValues{1}:="id-"+FT_NumberFormat($seqCust; 0; 3)
	...
	$tmp:=Substring:C12([PictureIDTypes:92]GovernmentCode:15; 1; 1)
	
	$element:=GAML_CreateXMLNode($identification; "type"; ->$tmp; True:C214)
	...
	//$tmp:=getCountryNameByCode ([Customers]PictureID_CountryCode)
	//$element:=GAML_CreateXMLNode ($identification;"country";->$tmp;False)
	...
	If (False:C215)
		...
		If ([Customers:3]isCompany:41)
			...
			$element:=GAML_CreateXMLNode($fullAddress; "country"; ->$tmp; False:C215)
			$seqAddr:=$seqAddr+1
	...
	$arrAttrValues{1}:="ind-"+FT_NumberFormat($seqInd; 0; 3)
	...
	If ([Invoices:5]ThirdPartyisInvolved:22)
		...
		$tmp:=getCountryNameByCode([ThirdParties:101]CountryCode:8)
		$element:=GAML_CreateXMLNode($mainAddress; "country"; ->$tmp; True:C214)
	...
	$tmp:="AUD"  //[Registers]Currency
	$element:=GAML_CreateXMLNode($totalAmount; "currency"; ->$tmp; True:C214)
	...
	// Money Received
	// --------------------------
	$moneyReceived:=GAML_CreateXMLNode($transaction; "moneyReceived")
	$arrAttrNames{1}:="id"
	...
	$element:=GAML_CreateXMLNode($foreignCash; "currency"; ->[Registers:10]Currency:19; True:C214)
	...
	$moneyProvided:=GAML_CreateXMLNode($transaction; "moneyProvided")
	$arrAttrNames{1}:="id"
	...
	$element:=GAML_CreateXMLNode($ausCash; "currency"; -><>baseCurrency; True:C214)
	...
	$fco:=GAML_CreateXMLNode($nonCashProvided; "fco")
	$element:=GAML_CreateXMLNode($fco; "amount"; ->$amount; True:C214)
