...
For each ($entity; $result)
	...
	Case of 
		...
		: ($method="KYC2020")
			...
			$3rdpartyStatus:=slOld_updateDisplayingLogStatus($3rdpartyStatus; $entity.CheckResult)
...
If ($result.length#0)
	...
	$form:=sl2_createDisplayResultOptions($selection; $showInterface)
	$selection.drop()  // fromCollection creates unwanted records
	$form.kyc2020:=$kyc2020
	$form.useCXR:=$useCXR
	$form.resultStatus:=slOld_updateDisplayingLogStatus($form.resultStatus; $3rdpartyStatus)
	$entity:=$result[0]
	If ($entity.isEntity)
...
// add default message
//If ($textPtr#Null) // <--- This crashed 4D v19R5 in compiled mode

If (Not:C34(Undefined:C82($textPtr->)))
