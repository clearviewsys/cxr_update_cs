...
C_BOOLEAN:C305($5; $isOutgoing; $continue)  // true: Outgoing, false: Incoming
...
While (Not:C34(End selection:C36([Wires:8])))
	...
	Case of 
		: ([Wires:8]isOutgoingWire:16)
			//if ([Wires]OriginatingBankCountryCode#[Wires]originatorCountryCode)
			
			If ([Wires:8]BeneficiaryBankCountryCode:77=<>CountryCode)
				$continue:=False:C215  // It is a Domestic EFT (send from CA to CA) doesn't include it
			End if 
			
			
		: ([Wires:8]isOutgoingWire:16=False:C215)  // Incoming EFT
			
			If ([Wires:8]originatorCountryCode:84=<>CountryCode)
				$continue:=False:C215  // It is a Domestic EFT (send from CA to CA) doesn't include it
			End if 
			
	End case 
	
	If ($continue=False:C215)  // Domestic EFT Detected
		
		// By Default Report doesn't include domestic ETF, else if the preference overwrite it.
		If (getKeyValue("FT.IncludeDomesticEFT"; "false")="true")
			$continue:=True:C214
		End if 
	...
	If ($continue)
		...
		If (Not:C34([Invoices:5]isAMLReported:45))
			...
			Else 
				...
				If (invoiceIncludesIRCurency([Invoices:5]InvoiceID:1))
