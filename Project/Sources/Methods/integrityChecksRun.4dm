...
If (Count parameters:C259>=1)
	...
	If ($obInfo.startDate=Null:C1517)
		$obInfo.startDate:=Add to date:C393(Current date:C33; -1; 0; -1)
	End if 
	
	If ($obInfo.endDate=Null:C1517)
		$obInfo.endDate:=Add to date:C393(Current date:C33; 0; 0; -1)
	End if 
	
Else 
	$obInfo:=New object:C1471("orphanRegisters"; "True"; "unbalancedInvoices"; "true"; "startDate"; Add to date:C393(Current date:C33; -1; 0; -1); "endDate"; Current date:C33)
End if 
...
If ($obInfo.orphanRegisters="True")
	...
	If (True:C214)
		$cInvoiceIDs:=ds:C1482.Invoices.query("CreationDate >= :1 AND CreationDate <= :2"; $obInfo.startDate; $obInfo.endDate).distinct("InvoiceID")
		
		$registers:=ds:C1482.Registers.query("CreationDate >= :1 AND CreationDate <= :2"; $obInfo.startDate; $obInfo.endDate)
		$registers:=$registers.query("not(InvoiceNumber in :1)"; $cInvoiceIDs)
...
If ($obInfo.unbalancedInvoices="True")
	...
	If (True:C214)
		$invoices:=ds:C1482.Invoices.query("CreationDate >= :1 AND CreationDate <= :2"; $obInfo.startDate; $obInfo.endDate)
	Else 
	...
	For each ($invoice; $invoices) While ($i<$invoices.length)
		...
		Else 
			If (Mod:C98($i; 100)=0)
				...
				Progress SET PROGRESS($iProgress; $iPercent; "Updating... "+String:C10($i)+" of "+String:C10($invoice.length))  //update the progress bar and the message
			End if 
		End if 
