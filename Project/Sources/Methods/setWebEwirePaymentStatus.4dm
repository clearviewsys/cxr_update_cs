...
C_BOOLEAN:C305($readOnlyStatus; $doUpdate)
C_TEXT:C284($gateway)
	...
	Case of 
		...
		: ($gateway="square")
			...
			Case of 
				...
				: ([WebEWires:149]paymentInfo:35.url#"")  //
					...
					If ([WebEWires:149]paymentInfo:35.transactionRefNo="")
						$return:=SQ_getOrderDetails([WebEWires:149]paymentInfo:35.payment_link.order_id)  //storing order id here
					Else 
		...
		: ($gateway="paymark")
			...
			Case of 
				...
				: (OB Get type:C1230([WebEWires:149]paymentInfo:35; "status")=Is undefined:K8:13)
					$doUpdate:=True:C214
					
				: ([WebEWires:149]paymentInfo:35.status="SUCCESSFUL@")
				: ([WebEWires:149]paymentInfo:35.status="CANCEL@")
				...
				: ([WebEWires:149]paymentInfo:35.status="NOT FOUND@")
				: ([WebEWires:149]paymentInfo:35.url#"")
					$doUpdate:=True:C214
				Else 
					$status:="UNDEFINED"  //no polilnk
			...
			If ($doUpdate)
				// check to see if we already have a status from paymark
