...
C_TEXT:C284($name; $otherName; $givenName; $fullName)
...
Case of 
	...
	: ([eWires:13]isBeneficiaryCompany:93=False:C215)
		...
		$fullName:=[eWires:13]BeneficiaryFullName:5
		$p:=Position:C15(" "; $fullName)
		
		If ($p>0)
			
			// surname
			$surname:=Substring:C12($fullName; $p+1; Length:C16($fullName))
			$surname:=FT_Clean($surname)
			$surname:=FT_StringFormat($surname; 20)
			TEXT TO BLOB:C554($surname; $content; UTF8 text without length:K22:17; *)
			
			// Given Name
			$givenName:=Substring:C12($fullName; 1; $p)
			$givenName:=FT_Clean($givenName)
