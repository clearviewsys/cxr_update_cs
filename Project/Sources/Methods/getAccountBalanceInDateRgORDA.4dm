...
If ($applyDateRange)
	...
	Else 
		$esAll:=ds:C1482.Registers.query("AccountID = :1 AND RegisterDate <= :2 AND BranchID = :3"; $accountID; $toDate; $branchID)  // The whole range up to toDate
	End if 
...
Else 
	If ($branchID="")
		$esRange:=ds:C1482.Registers.query("AccountID=:1"; [Accounts:9]AccountID:1)  // the range is the same as the whole thing
	Else 
		$esRange:=ds:C1482.Registers.query("AccountID=:1 AND BranchID = :2"; [Accounts:9]AccountID:1; $branchID)  // the range is the same as the whole thing
	End if 
