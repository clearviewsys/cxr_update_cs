...
If (OK=1)
	...
	For ($i; 1; Records in selection:C76([LinkedDocs:4]))
		...
		Case of 
			: ([LinkedDocs:4]filePath:26#"")
				$file:=DOC_retrieveDocumentPath([LinkedDocs:4]filePath:26)
				
			: (Picture size:C356([LinkedDocs:4]ScannedImage:2)>0)
	...
	If (True:C214)  // need to save attachements to server path for this to work
		sendNotificationByEmail($email; $subject; $body; $attachment)
		
		notifyAlert("Email"; "Email has been queued."; 5)
	Else 
