...
//**************************************************************
//Method for Finding Beneficiaries on the Currency Cloud Platform
//
//Required Parameters:
...
//
//Outputs:
//On Success: Satus code(200), @Array contains beneficiary IDs
...
If (Length:C16($authKey)>3)
	...
	$status:=HTTP Request:C1158(HTTP GET method:K71:1; \
		$URL+"/beneficiaries/find"; \
		$body; $response; $headerNames; $headerValues)
	
	CC_createLogRecord("GET beneficiaries/find"; "auth_token="+$authKey; String:C10($status); $response)
	...
	If ($status=200)
		...
		//Copies the beneficiary IDs to an array
		ARRAY TEXT:C222($IDs; 0)
		For ($i; 0; $oResponse.beneficiaries.length-1)
			APPEND TO ARRAY:C911($IDs; $oResponse.beneficiaries[$i].id)
		End for 
