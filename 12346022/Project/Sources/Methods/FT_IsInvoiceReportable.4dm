//%attributes = {}
// FT_IsInvoiceReportable ($invoiceNumber;$refDate;$exported)
...
//QUERY SELECTION([Registers];[Registers]RegisterDate>=Add to date($refDate;0;0;-1);*)
//QUERY SELECTION([Registers]; & ;[Registers]RegisterDate<=$refDate;*)
...
While (Not:C34(End selection:C36([Registers:10])))
	...
	//If (([Accounts]isCashAccount) & ([Registers]Currency=<>baseCurrency))
...
If ($totMarketRate>[ServerPrefs:27]twoIDsLimit:15)
