//%attributes = {}
// FT_Set24HoursIndicatorEFTO_NS ($dateref;->$invoicesPtr;->$invoices24hIndPtr)
...
FT_queryEFTO_NS($fromDate; $refDate; True:C214)
USE SET:C118("efto")
...
While (Not:C34(End selection:C36([Wires:8])))
	...
	If ([WireTemplates:42]BankCountryCode:35#"CA")  // Don't process domestic Wires
		...
		If (Not:C34([Invoices:5]isAMLReported:45))
			...
			If ((FT_SumUsingMarketRate_EFT([Wires:8]CXR_RegisterID:13)>=[ServerPrefs:27]twoIDsLimit:15) & ([Wires:8]WireTransferDate:17=$refDate))
			...
			Else 
				...
				If (([Wires:8]Currency:15="TOM") | ([Wires:8]Currency:15="IRR"))
					If ([Wires:8]WireTransferDate:17=$refDate)
						APPEND TO ARRAY:C911($arrRecordsPtr->; [Wires:8]CXR_WireID:1)
						APPEND TO ARRAY:C911($records24hIndPtr->; False:C215)
					End if 
