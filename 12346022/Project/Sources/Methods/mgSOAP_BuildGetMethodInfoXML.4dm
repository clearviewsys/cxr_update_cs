...
C_OBJECT:C1216($3; $arguments)  // arguments for MoneyGram $SOAP_Method we are about to get info for
...
If (OK=1)
	...
	$subelem:=DOM Create XML element:C865($root; $soapPrefs.root+"/soap:Body/GetMethodInfo"; $soapPrefs.namespaceName1; $soapPrefs.namespace1)
	
	// $subelem:=DOM Create XML element($root;$soapPrefs.root+$bodyPath)
	
	// $subelem:=DOM Create XML element($root;$soapPrefs.root+$bodyPath+"/"+$webPrefix+"systemId")
	$subelem:=DOM Create XML element:C865($root; $soapPrefs.root+"/soap:Body/GetMethodInfo"+"/"+"systemId")
	DOM SET XML ELEMENT VALUE:C868($subelem; "mg")
	
	// $subelem:=DOM Create XML element($root;$soapPrefs.root+$bodyPath+"/"+$webPrefix+"method")
	$subelem:=DOM Create XML element:C865($root; $soapPrefs.root+"/soap:Body/GetMethodInfo"+"/"+"method")
	DOM SET XML ELEMENT VALUE:C868($subelem; $SOAP_Method)
