...
If ($isTesting)
	$url:=getKeyValue("web.configuration.payments.square.test.url"; "https://connect.squareupsandbox.com/v2/online-checkout/payment-links")
...
Else 
	$url:=getKeyValue("web.configuration.payments.square.url"; "https://connect.squareupsandbox.com/v2/online-checkout/payment-links")
...
If (Count parameters:C259>=1)
	...
	If (True:C214)
		...
		If (Choose:C955($elem=-1; True:C214; False:C215))  //Amount is not in the object
			...
			End if 
		Else 
			$request.quick_pay.price_money.amount:=Num:C11(String:C10(Int:C8(OB Get:C1224($params; $aProperties{$elem})))+String:C10(Dec:C9(OB Get:C1224($params; $aProperties{$elem}))*100))
		End if 
...
If ($return.success)
	...
	APPEND TO ARRAY:C911($atHeaderNames; "Square-Version")
	APPEND TO ARRAY:C911($atHeaderValues; "2022-06-16")
	APPEND TO ARRAY:C911($atHeaderNames; "Authorization")
	APPEND TO ARRAY:C911($atHeaderValues; "Bearer "+$token+"")
	APPEND TO ARRAY:C911($atHeaderNames; "Content-Type")
	APPEND TO ARRAY:C911($atHeaderValues; "application/json")
	
	
	HTTP SET OPTION:C1160(HTTP timeout:K71:10; 120)
	
	$stat:=HTTP Request:C1158(HTTP POST method:K71:2; $url; $request; $response; $atHeaderNames; $atHeaderValues)
...
Case of 
	...
	Else   //ERROR WITH REQUEST
		$return.success:=False:C215
		$return.status:=$stat
		$return.statusText:=$response.errors[0].details
