...
// ------------------------------------------------------------------------------
// Method: FT_PartB1_STR: Information about transaction.
...
C_TEXT:C284($12)  // User created the Tx
C_BOOLEAN:C305($13)  // Related to Iran?

C_BLOB:C604($content)
C_TEXT:C284($partId; $partSeqNum; $txDate; $txTime; $nigthDepDropInd; $postingDate; $dAmount; $txCurrency; $txCode; $txCodeOtherDesc)
C_TEXT:C284($dFunds; $otherInst; $otherPerson; $txCode; $oDesc; $idPerson)
...
Case of 
	...
	Else 
		...
		$dFunds:="L"
		If ($13)
			$txCodeOtherDesc:="Transfer gov't Iran"
		Else 
			$txCodeOtherDesc:="CURRENCY EXCHANGE"
		End if 
...
$txCodeOtherDesc:=FT_StringFormat($txCodeOtherDesc; 20)
TEXT TO BLOB:C554($txCodeOtherDesc; $content; UTF8 text without length:K22:17; *)

If ($9=getKeyValue("TOMCode"; "TOM"))
	$dAmount:=FT_NumberFormat($8*10; 2; 15; "0"; "RJ")  // 1 Toman = 10 IRR
	$txCurrency:=FT_StringFormat("IRR"; 3)
Else 
...
// This is for additional information about the funds described in field B5 (Detail funds involved in initiating transaction)
// Provide the name and the identification number if applicable of any other instit.
// If more than one other was involved, put the information about the others in Part G
// this field is mandatory if applicable.

$otherInst:=" "
$otherInst:=FT_StringFormat($otherInst; 35)
TEXT TO BLOB:C554($otherInst; $content; UTF8 text without length:K22:17; *)


// Other person or entity account number
$otherPerson:=" "
$otherPerson:=FT_StringFormat($otherInst; 30)
...
// Other description
$oDesc:=FT_StringFormat($11; 20)
TEXT TO BLOB:C554($oDesc; $content; UTF8 text without length:K22:17; *)

// ID number of the person initially identifying a suspicious transaction
//$idPerson:=FT_StringFormat ($12;15)
$idPerson:=FT_StringFormat(" "; 15)
TEXT TO BLOB:C554($idPerson; $content; UTF8 text without length:K22:17; *)
