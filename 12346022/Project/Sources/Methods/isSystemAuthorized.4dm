	...
	Else 
		If (([MACs:18]ExpirationDate:2#!00-00-00!) & ([MACs:18]ExpirationDate:2-Current date:C33<=15))  // less than 2 weeks to expiry
			...
			Else 
				...
				sendNotificationByEmail(<>administratorEmail; "License expiration approaching"; getThisComputerBranchID+"-"+[MACs:18]ComputerName:19+" license will expire on "+String:C10([MACs:18]ExpirationDate:2))
			End if 
		End if 
	End if 
