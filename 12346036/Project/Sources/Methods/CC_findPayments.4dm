...
//**************************************************************
//Method for Finding Payments on the Currency Cloud Platform
//
//Required Parameters:
...
If (Length:C16($authKey)>3)
	...
	CC_createLogRecord("GET payments/find"; "auth_token="+$authKey; String:C10($status); $response)
	...
	If ($status=200)
		...
		//Copies the payment IDs to an array
		ARRAY TEXT:C222($IDs; 0)
		For ($i; 0; $oResponse.payments.length-1)
			APPEND TO ARRAY:C911($IDs; $oResponse.payments[$i].id)
		End for 
