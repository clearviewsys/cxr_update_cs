...
FT_queryEFT($fromDate; $refDate; True:C214)
USE SET:C118("efto")
...
While (Not:C34(End selection:C36([eWires:13])))
	...
	If (Not:C34([Invoices:5]isAMLReported:45))
		...
		If ((FT_SumUsingMarketRate_EFT([eWires:13]RegisterID:24)>=[ServerPrefs:27]twoIDsLimit:15) & ([eWires:13]SendDate:2=$refDate))
		...
		Else 
			...
			//If (([eWires]Currency="TOM") | ([eWires]Currency="IRR"))
			If (([eWires:13]Currency:12="TOM") | ([eWires:13]Currency:12="IRR") | ([eWires:13]fromCountry:9="IR") | ([eWires:13]toCountry:10="IR"))
				If ([eWires:13]SendDate:2=$refDate)
					APPEND TO ARRAY:C911($arrRecordsPtr->; [eWires:13]eWireID:1)
					APPEND TO ARRAY:C911($records24hIndPtr->; False:C215)
				End if 
...
//QUERY SELECTION([eWires];[eWires]SendDate=$refDate)
ORDER BY:C49([eWires:13]; [eWires:13]SendDate:2; >)
