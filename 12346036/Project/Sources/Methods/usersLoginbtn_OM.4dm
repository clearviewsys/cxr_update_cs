...
If ((arrUserNames>0) & (arrUserNames<4))  // system users picked were Designer, Administrator, or Support Staff
	...
	If (isDesignerEnabled)
		...
		If (Validate password:C638(arrUserNames{arrUserNames}; vPassword))
			...
			// CHANGE CURRENT USER(1;vPassword)  // this is successful sign-in for super users
			
			SET USER ALIAS:C1666(arrUserNames{arrUserNames})
	...
	If (isValidationConfirmed(True:C214))
		...
		If ($authenticationOK)
			...
			If (isValidationConfirmed(True:C214))
				...
				//7/11/22 @ibb -- this become the new current user
				...
				iH_registerClient(Current user:C182)  //IBB 1/28/20
