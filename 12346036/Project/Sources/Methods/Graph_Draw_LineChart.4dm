...
// Date and time: 10/28/20, 10:31:01
// ----------------------------------------------------
// Method: Graph_Draw_LineChart ($graphPtr;->$arrX;->$arrY;$oSettings)
// Description
// 
//
// Parameters
...
C_OBJECT:C1216($oSettings)  // graph settings
...
If (OB Is defined:C1231($oSettings))  // shouldn't be needed but...
	...
	If (Not:C34(OB Is defined:C1231($oSettings; "Graph line width")))
		$oSettings[Graph line width:K82:28]:=2  //  only for Line, default 2
	End if 
	...
	If (Not:C34(OB Is defined:C1231($oSettings; "Graph plot height")))
		$oSettings[Graph plot height:K82:26]:=12  //  only for Line, default 12
	End if 
	
	// Graph plot radius  //  only for Scatter, default 12
	
	If (Not:C34(OB Is defined:C1231($oSettings; "Graph plot width")))
		$oSettings[Graph plot width:K82:25]:=12  //  only for Line, default 12
	End if 
	...
	If (Not:C34(OB Is defined:C1231($oSettings; "Graph xProp")))
		// True for proportional x-axis; False for normal x-axis
		$oSettings[Graph xProp:K82:5]:=False:C215  //  only for Line, Surface, Scatter, default False
	End if 
