...
C_OBJECT:C1216($result; $transfer)
...
Case of 
			...
			If ([WebEWires:149]paymentInfo:35.passedToMoneyGram.transactionType="Send")
				...
				If (Not:C34([WebEWires:149]isCancelled:20))
					...
					If ([WebEWires:149]paymentInfo:35.origin="SOAP")
						...
						If ([WebEWires:149]status:16>=10)  // our internal status, not MoneyGram status
	...
	: (Form event code:C388=On Clicked:K2:4)
		...
		If (Form:C1466.paymentInfo.soap.result.result.TransferSysId#Null:C1517)
			...
			$transfer:=mgGetTransferByFormFreeStageNum($confirmationNumber; $sendAmount)
			...
			If ($transfer.success)
				...
				If ($transfer.result#Null:C1517)
					...
					If ($transfer.result.length>0)
						...
						$result:=mgSendConfirm($transfer.result[0].TransferSysId)
						...
						If ($result.success)
							...
							If ([WebEWires:149]paymentInfo:35.soap.getTransfer.result#Null:C1517)
								If ([WebEWires:149]paymentInfo:35.soap.getTransfer.result.length>0)
									...
									[WebEWires:149]paymentInfo:35.result.destinationCountry:=mgCountryID2CountryCode(Num:C11([WebEWires:149]paymentInfo:35.soap.getTransfer.result[0].ReceiverCountry))
								End if 
							End if 
