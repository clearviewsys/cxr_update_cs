//%attributes = {}
//  matchThisInvoice_vs_AMLRule (thisInvoiceObj; customerObj; ruleObj ; inSimulator) -> isMatch: boolean
...
If ($customerObj#Null:C1517)
	...
	If ($inSimulator)  // this is for the prototyper
		$filterObj:=newAML_AggrRuleFilter  // need to change this to map the rule to a filter 
	Else 
		$filterObj:=newAML_RuleFilter($ruleObj; $customerID; $thisInvoiceID; $group)
	End if 
	...
	If (($thisInvoiceID="") & (Num:C11($ruleObj.isByInvoice)=1))  // if the rule is by invoice and no invoiceID is provided then not a match
		$match:=False:C215
	End if 
	$match:=($match & (matchCustomer_vs_AMLRule($customerObj; $statsObj; $ruleObj)))
