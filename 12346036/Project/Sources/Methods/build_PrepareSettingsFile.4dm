...
build_dumpVar2File($old_xml; "old.xml")  // dump template to artifacts folder for debugging
...
For each ($oneaction; $actions)
	...
	If ($oneaction="BUILD_APP")
		...
		If ($parameters.pathToVL=Null:C1517)
			...
			End if 
			If (Is Windows:C1573)
				$parameters.pathToVL:=System folder:C487(Documents folder:K41:18)+"4D Volume Desktop\\"
			End if 
		Else 
			If (Is macOS:C1572)
...
// TEXT TO DOCUMENT(System folder(Documents folder)+"artifacts"+Folder separator+"buildApp.4DSettings"; $new_xml; "UTF-8")  // upload this via artifact upload github action

build_dumpVar2File($new_xml; "buildApp.4DSettings")  // dump setting to artifacts folder for debugging
