...
//fills the report for aggregate Invoice summary for cash transactions
//author: Amir
//24th August 2018
...
ARRAY TEXT:C222(arrInvoiceIDs; 0)  // clear the invoice
...
//QUERY SELECTION([Registers];[Registers]DebitLocal>0)  // receive only
...
$progress:=launchProgressBar("Calculating cash summary of invoices...")

ORDER BY:C49([Registers:10]; [Registers:10]InvoiceNumber:10; >)  // sort registers by their invoice ID
...
C_BOOLEAN:C305($isSameInvoice)

Repeat 
	
	$isSameInvoice:=($previousInvoiceID=[Registers:10]InvoiceNumber:10)
	...
	Else 
		...
		//If ((Not([Customers]isInsider)) & ($tempDebitSum>=<>TWOIDSLIMIT))  // conditions met
		If ($tempDebitSum>=<>TWOIDSLIMIT)  // conditions met
			...
			arrInvoiceIDs{$j}:=$previousInvoiceID
			If ($showCustomerNames)
...
If ((Not:C34([Customers:3]isInsider:102)) & ($tempDebitSum>=<>TWOIDSLIMIT))  // conditions met
