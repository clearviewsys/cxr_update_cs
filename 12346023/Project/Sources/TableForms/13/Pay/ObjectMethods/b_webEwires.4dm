...
Case of 
	: (Form event code:C388=On Load:K2:1)
		...
		If (Is new record:C668([eWires:13]))
			$count:=selectActiveWebeWires4Customer([Customers:3]CustomerID:1)
	...
	: (Form event code:C388=On Clicked:K2:4)
		...
		$currWebEWireID:=[eWires:13]WebEwireID:123
		
		$WebEWireID:=pickWebEWireForInvoice
		If ($WebEWireID#"")
			$error:=createEwireFromWebeWire($WebEWireID)
