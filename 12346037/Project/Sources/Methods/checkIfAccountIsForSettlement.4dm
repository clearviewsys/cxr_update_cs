...
Case of 
	: ($2=True:C214)  // we want a bank account so error is if record is not a bank account
		If (Not:C34([Accounts:9]isSettlementAccount:25))  // is not a bank account
			checkAddWarningOnTrue((Count parameters:C259=2); $account+" is not assigned for settlements. ")
		End if 
	: ($2=False:C215)  // we don't want a bank account so give the error if it is a bank account
		If ([Accounts:9]isSettlementAccount:25)  // is not a bank account
			checkAddWarningOnTrue((Count parameters:C259=2); $account+" is a settlement account. Integrity Warning!")
		End if 
End case 
