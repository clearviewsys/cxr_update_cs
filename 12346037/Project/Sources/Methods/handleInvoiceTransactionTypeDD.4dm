//%attributes = {}
// handleTransactionTypeDropDown (self; ->[invoices]TransactionType;{form event})
...
Case of 
	...
	: ($formEvent=On Load:K2:1)  // load the combo box array with transaction types
		If (isUserManager)
			ALL RECORDS:C47([TransactionTypes:93])
		Else 
			QUERY:C277([TransactionTypes:93]; [TransactionTypes:93]isRestrictedToManagers:5=False:C215)
		End if 
		ORDER BY:C49([TransactionTypes:93]; [TransactionTypes:93]TransTypeID:1)  // order by Trans Type
		SELECTION TO ARRAY:C260([TransactionTypes:93]TransTypeID:1; $self->)
		// once the form is loaded, assign the default value of the box to the field
		$index:=Find in array:C230($self->; [Invoices:5]TransactionTypeID:91)
		If ($index>0)
			$self->:=$index
			$self->{$index}:=[Invoices:5]TransactionTypeID:91
		End if 
